...
- **Gizmo 从单位位置直接连到 `waypoints[CurrentIndex]`，画出笔直穿过整个地图的线。**

  `CurrentIndex` 是「正在赶往的那个航点」，路径一经平滑常在二十几米外 —— 于是屏幕上
  出现一条穿过大片禁区的直线。**那不是路径**：单位沿折线走，前瞻点只取折线上前方
  一米处，从不走这条线。这条假线误导性极强，看着就像「规划穿过了禁区」。

  改成从**折线上的投影点**起画（与 `NavPathFollower.LookAhead` 同一口径），
  单位到投影点的横向偏离单独用速度色画一段短的。画出来的就是它实际跟的那条线。

### Changed

- 评估过在 `NavPathFollower` 上加「横向偏离就先回路径」的规则，**放弃**：
  判据无法与正常纯追踪区分 —— 实测故障现场只偏离 0.39 米，而两个既有用例
  故意构造了 1 米与 2 米的偏离并断言仍走前瞻。真正的约束是「往某个轴走会被挡住」，
  那是**移动层**的事（消费方按体素边界削轴即可），跟随器手里没有可走性，判不了。

## [0.2.23] — 通视改超覆盖遍历，对角步不许切角

### Fixed

- **平滑后的路径会擦着不可走体素过，代理沿它走会踩进去。**

  `HasLineOfSight` 按「半个体素边长」点采样判通视。线段只要在某个不可走体素里
  掠过一小段（比采样步长短），采样点就会整体跨过去、判成通视。

  这不是偶发：8 邻接的对角步长 `0.5×√2`，**中点精确落在四个体素共享的角上** ——
  正是最容易擦到被堵角落的位置；而按半格采样时采样点落在 1/3、2/3 处，
  恰好跨过那个中点。

  实测（消费工程）：23 条路径边里 **7 条按代理半径走不通，按旧口径全部判成通视**。
  代理沿这条线走会踩进不可走体素，被自己的防穿墙层拦下，站在原地再也过不去 ——
  外部看到的就是「卡弯」。

  改成**超覆盖遍历**：逐格点出线段经过的每一个体素；恰好从格点穿过时，
  把这些轴的所有非空组合格子都查一遍（最多 7 格）。格对齐几何下这条路径是常态，
  不是例外。

- **A* 与流场的对角步允许「切角」。**

  一次对角位移只校验终点可走，于是搜索会给出从两堵墙的夹角斜切过去的路径：
  两端都可走、中间擦过的那一格不可走，半径大于零的代理根本过不去。

  现在对角位移会把它擦到的所有**中间格**（分量的非空真子集：2D 两个、3D 六个）
  一并校验。`NavAStar.Relax`、`NavFlowFieldSolver.Relax`、`NavFlowFieldSolver.TryGetNext`
  三处都补上了，分解逻辑收在 `NavCorner`。

  这条不变式还有第二个消费者：`PullString` 退回「最低等级」那趟兜底，
  前提是「相邻航点在同一等级的 A* 里必然相连」。A* 允许切角时这条前提就破了 ——
  兜底那趟也通视失败，整条请求被判 Failed。

### Tests

- 新增 `NavPathSmootherTests`：夹角斜切、单侧邻居被堵、正交步不受影响，
  以及端到端「A* 路径的相邻航点必然通视」。

## [0.2.22] — 前瞻从「进来的那一段」投影，不再斜切弯角

### Fixed

- **代理还差一点没到当前航点时，期望速度会从侧面切出路径。**

  0.2.20 让前瞻先把位置投影到折线上。但投影只扫「当前航点**及之后**」的段，
  而 `currentIndex` 是「正在赶往的那个航点」—— 代理通常还差一点没到它。
  于是投影被迫落在该航点本身，前瞻再从那里沿**下一段**走，方向就从侧面切了出去：

  ```
  直角路径 (0,0)→(0,10)→(10,10)，代理在 (0,5) 正沿直路段前进、当前航点 (0,10)
    → 期望方向 (0.37, 0.93)   ← 斜切向拐角的另一侧
  应为 (0, 1)                 ← 先笔直走到拐角
  ```

  战场上表现为：路径本身没问题（实测全程净空 0.75 米），期望方向却把代理推向路径
  侧面的低净空带 —— 进去、被位置投影弹出、再进去，就是弯角处的反复抖动。

  修复：投影改从「当前航点的**前一段**」开始扫（`clamp(currentIndex - 1, 0, lastSegment)`）。
  `currentIndex` 是目标航点，通向它的那一段才是代理脚下这一段。
